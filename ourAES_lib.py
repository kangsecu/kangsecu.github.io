...
def block2str(in_block):
	...
	for i in range(len(in_block)):
		out_string = out_string + "%02x" % in_block[i]
	return out_string
	
def str2block(in_string):
	out_block =[]
	tmp_string = in_string
	for i in range(int(len(in_string)/2)):
		out_block.append(int(tmp_string[:2],16))
		tmp_string = tmp_string[2:]
	return out_block
	
def hex_block_print(in_block):
	print("[ ", end="")
	for i in range(16):
...
def Padding(pt):
   if(len(pt) % 16 ==0):
      return pt #패딩이 블록의 배수인 경우 원래 평문 return
   else:
      block_size = 16
      pad_num = block_size - (len(pt) % block_size) # 패딩할 바이트 수
      block_num = (len(pt) + block_size) // block_size # 패딩된 평문의 블록 개수
      pad_pt = [0 for i in range(block_num * block_size)] # 패딩된 평문이 저장될 공간
      pad_pt[0:len(pt)] = pt # 원래 평문
      pad_pt[len(pad_pt)-1] = pad_num # 마지막 바이트에 패딩 추가
      return pad_pt
...
def main(): 
	block = [ 0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d,0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x07, 0x34 ]#plaintext 
